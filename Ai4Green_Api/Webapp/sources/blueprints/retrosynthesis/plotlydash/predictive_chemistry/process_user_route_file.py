...
from .utils import (
    compound_from_inchi,
    encodings_to_smiles_symbols,
    sig_figs_on_numbers,
    smiles_to_inchi,
)


def read_user_route_file(contents, filename):
...
def process_optional_columns(df: pd.DataFrame):
    """
...
class Conditions:
    def __init__(self, step_dataframe, reactants_smiles, product_smiles):
    ...
    def to_dict_array(self):
        ...
        self.predicted_conditions = [conditions_set]
        self.process_conditions()
        return self.predicted_conditions

    def to_dict_array2(self):
        step_df = self.step_df
        conditions_set = {
            "score": 1,
...
class RetroRoute:
    ...
    def to_dict_arrays(self):
        ...
        for product in products:
            ...
            self.condition_sets.append(
                {self.node_id_ls[-1]: conditions_object.to_dict_array2()}
            )
