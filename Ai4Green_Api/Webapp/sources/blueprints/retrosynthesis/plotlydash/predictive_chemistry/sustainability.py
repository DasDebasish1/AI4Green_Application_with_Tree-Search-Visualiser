...
from .utils import compound_from_id, solvent_from_name
...
class ReactionSustainabilityFlags:
    ...
    def get_solvent_flag(self):
        ...
        for solvent in solvents:
            if solvent in ["No Compound", "Not Found"]:
                # no solvent means a score of 1.
                solvent_flag_score = 1
            else:
                solvent = solvent_from_name(solvent)
                if solvent != "Not Found":
                    solvent_flag = solvent.flag
                else:
                    solvent_flag = 5  # Non Chem21 solvent
    ...
    def get_atom_economy_flag(self):
        # Requires equivalents assumption. - Could atom counting on either side work?
    ...
    def get_safety_flag(self):
        ...
        for compound_type in compounds:
            ...
            for compound_id in compound_id_ls:
                if compound_id in ["No Compound", "Not Found"]:
                    continue
                compound_object = compound_from_id(compound_id)
                compound_hazard_codes = compound_object.hphrase
                if compound_hazard_codes is None:
                    continue
...
class RouteSustainabilityFlags:
    ...
    def __init__(self, reaction_list: List[dict]):
    ...
    def get_sustainability_flags(self):
        for reaction_dic in self.reaction_list:
            for reaction_sustainability in reaction_dic.values():
                if reaction_sustainability == "Condition Prediction Unsuccessful":
                    continue
                top_conditions = reaction_sustainability[0]
...
def get_sustainability_for_all_routes(all_conditions: List[dict]) -> dict:
    all_routes_sustainability_list = []  # append each route
    all_routes_sustainability = {}
    print("sustainability assessment func")
    for route in all_conditions:
        # get a list of the steps for a route
        route_label = list(route.keys())[0]
        route_steps = route[route_label]
        route_sustainability_list = []  # append each node/step
        for reaction_step in route_steps:
            # assess sustainability for each step
            reaction_step_label = list(reaction_step.keys())[0]
            reaction_step_value = reaction_step[reaction_step_label]
            if reaction_step_value == "Condition Prediction Unsuccessful":
                route_sustainability_list.append(
                    {reaction_step_label: "Condition Prediction Unsuccessful"}
                )
                continue
            reaction_step_sustainability_list = []  # append each condition set
            for conditions in reaction_step_value:
                # assess sustainability for each condition set for a reaction
                # 1 is most sustainable, 4 the least.

                sustainability = ReactionSustainabilityFlags(conditions)
                ...
                # append the sustainability scores for the condition set
                reaction_step_sustainability_list.append(sustainability_dict)
            # append the list of conditions for a reaction step
            route_sustainability_list.append(
                {reaction_step_label: reaction_step_sustainability_list}
            )
        # append the list of steps for a route
        route_sustainability = RouteSustainabilityFlags(route_sustainability_list)
        route_sustainability.get_sustainability_flags()
        route_flags = route_sustainability.to_dict()
        all_routes_sustainability_list.append(
            {
                route_label: {
                    "steps": route_sustainability_list,
                    "route_average": route_flags,
                }
            }
        )
        all_routes_sustainability.update(
            {
                route_label: {
                    "steps": route_sustainability_list,
                    "route_average": route_flags,
    ...
    return all_routes_sustainability
...
def weighted_median_for_each_step(
    ...
    Each condition set dictionary is altered and the unweighted median replaced by the weighted median for that conditon set
    """

    for step in route_sustainability["steps"]:
        if list(step.values())[0] == "Condition Prediction Unsuccessful":
            continue
        for condition_set in step[list(step.keys())[0]]:
            values_ls = [
                val["flag"]
                for key, val in condition_set.items()
                if key != "unweighted_median"
            ]
            df = pd.DataFrame({"value": values_ls, "weight": sustainability_weightings})
