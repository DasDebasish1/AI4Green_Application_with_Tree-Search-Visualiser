...
from sources.blueprints.retrosynthesis.plotlydash.predictive_chemistry.utils import (
    smiles_to_inchi,
)
...
def reaction(
    ...
    Args:
        ...
        conditions_dropdown_value: The selected condition set

    Returns:
        dbc.Table: Table containing reaction conditions and sustainability information.
    """
    selected_condition_idx = (
        int(conditions_dropdown_value[-1]) - 1
    )  # minus 1 to use zero-based index
    conditions_data = conditions_options[selected_condition_idx]
    sustainability = sustainability_options[selected_condition_idx]
    ...
    table = dbc.Table(
        table_header + table_body, bordered=True, id="reaction-conditions-data-table"
    )
    return table
...
def make_reaction_table_rows(
    ...
    """
    score_row = generate_html_row("Likelihood Score", conditions_data["score"])
    ...
    solvent_row = generate_html_row(
        "Solvent",
        conditions_data["solvent_names"],
        sustainability["solvent"]["style"],
    )
    reagents_row = generate_html_row("Reagents", conditions_data["reagents_names"])
    catalyst_row = generate_html_row(
        "Catalyst",
        conditions_data["catalyst_names"],
    ...
    safety_row = generate_html_row(
        "Safety",
        ", ".join(sustainability["safety"]["hazard_code_list"]),
...
def routes(
    ...
    """
    Generates tables for route statistics and sustainability.
    ...
    for step_sustainability in route_sustainability["steps"]:
        sustainability_dict = list(step_sustainability.values())[0][0]
        node_key = list(step_sustainability.keys())[0]
        get_table_cell_styles(sustainability_dict)
        sustainability_dict.update({"node_id": node_key})
        processed_sustainability.append(sustainability_dict)
    ...
    for row_key in row_keys:
        new_row = [
            html.Td(row_key.replace("_", " ").title(), style={"border-color": "black"})
        ]
...
def generate_html_row(label: str, value: str, style: Optional[Dict] = None) -> html.Tr:
    ...
    """
    default_style = {"border-color": "black"}
    if style is None:
        style = default_style
    return html.Tr(
        [
