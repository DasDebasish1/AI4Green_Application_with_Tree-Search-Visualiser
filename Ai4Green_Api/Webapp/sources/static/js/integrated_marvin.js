...
function sketcherEventListeners(marvin){
    ...
    document.getElementById('retrosynthesis-export').addEventListener('click', function(){
        exportDataToRetrosynthesis(marvin)
    })
}

async function exportDataToRetrosynthesis(marvin){
            // export product from sketcher to retrosynthesis
            let smiles = await exportSmilesFromSketcher(marvin)
            if (smiles.includes(">>")) {
