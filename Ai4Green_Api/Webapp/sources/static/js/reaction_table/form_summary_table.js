...
function showSummary() {
    ...
    if (tutorial_mode !== 'yes') {
        if (reactionDiv.childNodes.length !== 0) {
            const text = "Please note that any summary table data already inputted will be lost. Do you wish to continue?";
    ...
    let reactantID, reactantMolecularWeightID, reagentMolecularWeightID, reactantMassID,
        reagentMassID, reactantDensityID, reactantConcentrationID, reactantEquivalentID,
        reactantAmountID, roundedReactantAmountID, reactantVolumeID, roundedReactantVolumeID,
        roundedReactantMassID, reagentDensities, productID, productMolecularWeightID,
        productMassID, roundedProductMassID, productHazardID, productPhysicalForm,
    ...
    for (let i = 1; i <= numberOfReactants; i++) {
        ...
        reactantPhysicalForms += $(reactantPhysicalFormID).val().replace(/ *\([^)]*\) */g, "") + ";";
    ...
    for (let i = 1; i <= numberOfReagents; i++) {
        ...
        reagentPhysicalForms += $(reagentPhysicalFormID).val().replace(/ *\([^)]*\) */g, "") + ";";
    ...
    for (let i = 1; i <= numberOfProducts; i++) {
        ...
        productPhysicalForms += $(productPhysicalFormID).val().replace(/ *\([^)]*\) */g, "") + ";";
    ...
    $.ajax({
        ...
        success: function(response){
            if (response.summary === 'Ensure you have entered all the necessary information!'){
...
function exportImage() {
    ...
    let $image = $("#image")
    $image.attr("src", "");
    let image_source = $("#js-reloaded-image").val()
