...
def _process_single_smiles(
    ...
    try:
        ...
        return
    finder.tree_search(show_progress=True,iterations=iterations)
    finder.build_routes()
    ...
    tree=finder.tree.root.to_dict(finder.analysis.best(),finder.routes)
    #json.dump(tree,open("tree.json","w"))
    stats = {
            "target": smiles,
            "search_time": finder.search_stats["time"],
            "first_solution_time": finder.search_stats.get("first_solution_time", 0),
            "first_solution_iteration": finder.search_stats.get(
                "first_solution_iteration", 0
            ),
        }
    stats.update(finder.analysis.tree_statistics())
  
    logger().info(stats_str)
    with open("aa.txt","w") as tes:
        tes.write(str(len(finder.routes.nodes)))
