...
class MctsNode:
    ...
    def from_dict(
        ...
        if "created_at_iteration" in dict_:
            node.created_at_iteration = dict_["created_at_iteration"]
    ...
    def to_dict(self,best,routes):
        mols=[]
        for i,mol in enumerate(self.state.mols):
        
            mols.append([mol.smiles,self.state.stock_availability[i]])
        children = []
        for child in self.children:
            children.append(child.to_dict(best,routes))
        reation_name="undefined"
        reaction_smile=""
        if self.parent:
            action=self.parent[self]["action"]
            reation_name=action.metadata["classification"]
            try:
                parent = self.parent
                if parent is not None:
                    reaction_smile = parent[self]["action"].mapped_reaction_smiles()
                #reaction_smile = self.parent._children_actions[self.parent.children.index(self)].mapped_reaction_smiles()
            except:
        ...
        return {"iteration":self.created_at_iteration,"reaction_smiles":reaction_smile,"routeno":self.routeno,"mols":mols,"reaction":reation_name,"children":children,"is_solved":self.inroute,"is_best":True if (self==best and self.is_solved and self.inroute) else False}
